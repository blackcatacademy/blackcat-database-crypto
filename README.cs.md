...
For the English README, see `README.md`.
...
Pro transform-only použití (např. testy, queue, offline tooling) viz `example.php` a `docs/INTEGRATIONS.cs.md`.
...
Pro volitelnou kontrolu proti živému schématu připrav JSON dle [docs/SCHEMA.cs.md](./docs/SCHEMA.cs.md). `db-crypto-plan` pak zvýrazní sloupce, které v DB chybí, a v CI vrátí nenulový exit kód.
...
Praktické integrační poznámky (např. `blackcat-auth`) jsou v [docs/INTEGRATIONS.cs.md](./docs/INTEGRATIONS.cs.md).
